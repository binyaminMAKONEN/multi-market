...
function ProductCard(props) {
  ...
  <div class="rounded-lg shadow-lg  border-red  border bg-white max-w-sm ">
    <a href="#!">
      <img class="rounded-t-lg w-fit"  src={props.img} alt=""/>
    </a>
    <div class="p-6">
      ...
      {props.quantity==0?<p className="text-right ">אזל במלאי</p>:
        <div className="flex justify-around ">
          <button className=" rounded-full w-6 text-lime-500 hover:text-white hover:bg-lime-500 " onClick={()=>{if(qty<10&&qty<props.quantity )setQty(qty+1)}}>+</button><span>{qty}</span><button className=" rounded-full text-lime-500 hover:text-white hover:bg-lime-500 w-6" onClick={()=>{if(qty>0)setQty(qty-1)}}>-</button>
        </div>}
